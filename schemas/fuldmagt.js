...
const verifyCreateFuldmagt = Joi.object({  
    ...
    agentName: Joi.string().when('receiverId', {
        is: '',
    ...
    agentDOB: Joi.date().iso().when('receiverId', {
        is: '',
    ...
    agentEmail: Joi.string().email().when('receiverId', {
        is: '',
    ...
    agentCountryCode: Joi.string().min(2).max(4).when('receiverId', {
      is: '',
    ...
    agentPhoneNumber: Joi.string().when('receiverId', {
      is: '',
      then: Joi.required().messages({
  ...
  module.exports = {verifyCreateFuldmagt}